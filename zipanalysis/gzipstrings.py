#!/usr/bin/env python
"""
hacked version of pyflate.py with bzip2 section removed based on:
...
def bwt_reverse(L, end):
    ...
    if len(L):
        ...
        # if there has been enough output generated.  I hadn't figured
...
def gzip_main(field):
    ...
    while True:
        ...
        if verbose: print "deflate-blocktype", blocktype, 'beginning at', header_start
        ...
        if blocktype == 0:
            ...
            if blocktype == 1: # Static Huffman
                if verbose: print "static huffman"
                ...
                if verbose: print "dynamic huffman"
            ...
            if verbose: print main_literals, main_distances

            literal_count = 0
            literal_start = 0
            # new: instead of just outputting the original keep stats on each chunk
            chunks = {}
            ...
            while True:
                ...
                if 0 <= r <= 255:
                    ...
                    literal_count += 1
                    out += chr(r)
                    if verbose: print 'found literal', `chr(r)`, "at",len(out)
                    if verboselist:
                            print chr(r), offset
                            chars.append({'offset': offset, 'zipchar':chr(r), 'zipseq': 0})
                            offset += 1
                elif r == 256:
                    if literal_count > 0:
                    ...
                    if verbose: print 'end of Huffman block encountered'
                    break
                elif 257 <= r <= 285: # dictionary lookup
                    if literal_count > 0:
                        if verbose: print 'add 0 count', literal_count, 'bits', lz_start-literal_start, 'data', `out[-literal_count:]`
                        literal_count = 0
                    length_extra = b.readbits(extra_length_bits(r))
                    length = length_base(r) + length_extra
                    if verbose: print 'dictionary lookup: length', length,
                    ...
                    if 0 <= r1 <= 29:
                        ...
                        if verbose: print 'copy', -distance, 'count', cached_length, 'bits', b.tellbits() - lz_start, 'data', `out[-cached_length:]`
                        if verboselist: 
                                start = "start"
                                for c in out[-cached_length:]:
                                        chars.append({'offset': offset, 'zipchar': c, 'zipseq': 1 if start == 'start' else 2 })
                                        print c,offset,start
                                        start = "body"
                                        offset += 1

                        chunk = out[-cached_length:]
                        if chunk not in chunks:
                                chunks[chunk] = 1 # at least one instance before this one
                        chunks[chunk] += 1
    ...
    if verbose: print 'deflate-end-of-stream'
...
def gunzip_file(filename):
    """
    reads a gzipped file and returns either chars or chunks depending on 
    the verboselist global - may also print lots if verbose is true
    """
    input = open(filename)
